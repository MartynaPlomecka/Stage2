...
Load the data:
```{r}
koncowa <- read.table("/Users/mplome/data/od50agg.csv",
                      header = TRUE, sep=",")
koncowa$id <- factor(koncowa$id)
koncowa$typ <- factor(koncowa$typ)
koncowa$age <- factor(koncowa$age)
koncowa$test_nr <- factor(koncowa$test_nr)
...
#best
lmreactioninter <- brm(rt~ 1 + typ*age + (1|id), 
                       data = koncowa)
...
anti_vs_pro = fit_lmreactioninter[,"0"] - fit_lmreactioninter[,"1"] 
...
#this is basically the MCMC chain but converted from regression coefficients for fitted values for each age. 
#for old, differences in the reaction time between old and young people
anti_vs_pro = fit_lmreactioninter_yng[,"0"] - fit_lmreactioninter_yng[,"1"] 
